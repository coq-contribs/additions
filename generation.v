...
Qed.
(*
 Realizer <Code>rec REC :: :: {Call_lt}
     [c:Call]
       <Code>Case c of
        (* Call_M p *)
         [p:nat]
         (COND3 Code p M3:: ::{(gencode (Call_M three))} 
                       [q:nat](M2q q)
                       [q:nat] (C2M p q (REC (Call_C p q))))
        (* Call_C p n0 *)
         [p,n0:nat]
         <Code>let (q,r:nat)=(eucl_dev n0 p) in
               <Code>if (zerop r) 
                     then 
                      (MMC n0 q (REC (Call_M n0)) (REC (Call_M q)))
                     else 
                      (KMC n0 q r (REC (Call_K n0 r)) (REC (Call_M q)))
       (* Call_K p n0 *)
        [p,n0:nat]
           <Code>let (q,r:nat)= (eucl_dev n0 p) in
               <Code>if (zerop r) 
                     then
                      (MMK n0 q (REC (Call_M n0)) (REC (Call_M q)))
                     else
                      (KMK n0 q r (REC (Call_K n0 r)) (REC (Call_M q)))
        end.
  (* ---------------------------------------------------------------- *)
  Do 2 Program.
  Exact Wf_Call_lt.
  ...
  Program.
  (* 
  A:  code generation for (Call_M p)) *)

  Intro H.
  Program_all.

  (* A1: Call to (Call_ C p q0) ( 2<= q0 < p)  *)
  (*  (Call_lt (Call_C p q0) (Call_M p)) *)
  (Constructor 2; Auto with arith).
  Elim a0; Auto with arith.
  (*  (conform (Call_C p q0)) *)
  (Simpl; Auto with arith).
  Elim a0; Auto with arith.

  (* A2: p is q0 power of 2 *)
  (Rewrite -> e; Auto with arith).

  (* A3: p=3 *)
  (Intros H' eq3; Rewrite -> eq3; Program_all).

  (* 
  B: code generation for (Call_C n n0) *)

  Do 3 Program.
  Intro H0.
  Simpl in H0;Elim H0;Intros H1 H2.
  Program.
  Program.
  Unfold gt ;Apply lt_trans with (S O) ;Auto with arith.
  Program.
  Program.
  Program.
  Intros e0.
 ...
 (* B1:  n0 divides n *)
  ...
  (* (Call_lt (Call_M n0) (Call_C p n0)) *)
  Constructor 3;Auto with arith.

  (* (conform (Call_M n0)) *)
  Simpl;(Elim e0;Auto with arith).
 
  (* (Call_lt (Call_M q) (Call_C p n0)) *)
  Constructor 3. 
  Rewrite -> eg;Rewrite -> mult_sym;Apply mult_p_lt_qp;Auto with arith.
 
  (*  (Spec (Call_C p n0) c0) *)
  Rewrite -> eg;Auto with arith.
 
  (* lt O q *)
  Apply quotient_positive with p n0.
  Auto with arith.
  Apply le_lt_trans with n0;Auto with arith.   
  ...
  (* (Call_lt (Call_K n0 r) (Call_C p n0)) *)
  Constructor 5;Auto with arith.
  ...
  (*  (conform (Call_M q)) *)
  Simpl.
  Apply lt_O_q with n0 r ;Auto with arith.
  Elim e;Auto with arith.
  Rewrite -> e;Auto with arith.

  (* 
  C:  code generation for (Call_K p n0) *)
 
 
   Do 3 Program.
   Intros H0. (* (conform (Call_K p n0)) *)
   Elim H0;Intros.
   Program.
   Program_all.
   Do 3 Program.
  ...
  (* (Call_lt (Call_M n0) (Call_K p n0)) *)
  Constructor 4;Auto with arith.
  
  (* (conform (Call_M n0)) *)
  Simpl;Auto with arith.
 
  (* (Call_lt (Call_M q) (Call_K p n0)) *)
  Constructor 4.
  Rewrite -> eg;Apply mult_p_le_pq;Auto with arith.
  ...
  Rewrite -> e;Rewrite -> e0;Rewrite <- plus_n_O;Auto with arith.

  (* C2: p=qn0+r (r>0)  *)
 
  Program_all.

  (* (Call_lt (Call_K n0 r) (Call_K p n0)) *)
  Constructor 6;Auto with arith.
  ...
  (* (Call_lt (Call_M q) (Call_K p n0)) *)
  Constructor 4;Auto with arith.
  Rewrite -> e;Apply le_q_qbr;Auto with arith.
 
  (* (conform (Call_M q)) *)
  Simpl.

  (* lt O q *)
  Apply lt_O_q with n0 r ;Elim e;Elim H0;Auto with arith.
  Rewrite -> e;Auto with arith.
 Qed.
*)
