...
(*
  Apply Imperative with state init ok;[
     ...
     (* :init *)
           Realizer (mkstate n0 O true) |
     (* :body *)
     (Intros  a H;
       Apply while_not with init invar stat_order final a;[
        (* :test *)  
               Realizer [s:state]
...
Auto with arith.                       
 ...
 (*
  ...
  pos_n0 : (lt O n0)
  ============================
   (a:state)(init a)->{a':state | (ok a')}
*)
 (* termination test : *)
   
      Program_all.
      Simpl.
      Change (lt O state_n0).
      Apply invar_inv3 with state_p0 state_b0;Auto with arith.
      Unfold final;Simpl in l;Auto with arith.
      Red;Intro.
      Absurd (lt (S O) one);[Auto with arith|Idtac].
      Rewrite <- (final_inv ? ? ? H1);Exact l.
      Simpl in e;Rewrite <- e;Split;Auto with arith.

  (* one loop step: *)

  Program.
  Do 3 Program.
  Intros  no_final state_ok .
  Cut (lt one state_n0).
  Intro n_ge_1.
  2:Apply not_final with state_p0 state_b0;Auto with arith.
  Program.
  Program.
  Program. 
  ...
  (*  b (state_b0) is true ,
   which means that until now all the quotients by 2 were exact,
   or more precisely:  *)
  Cut n0=(mult (two_power state_p0) state_n0).
  Intro n0_2pn.
  2:Apply invar_inv1;Elim H;Auto with arith.
  Simpl in H.
  Simpl.
  Program.
  Program.
  ...
  Rewrite  two_power_S.
(*  Rewrite (mult_sym two (two_power state_p0)). *)
  Rewrite <- mult_assoc_l;Auto with arith.
  Unfold shift in e;Elim e;Auto with arith.
  Apply quotient_positive with state_n0 two .
  Rewrite mult_sym;Exact e.
  Apply lt_trans with one ;Auto with arith.
  Unfold stat_order ltof.
  Apply half_lt.
  Apply lt_trans with one ;Auto with arith.
  Auto with arith.
...
(*  Rewrite -> (mult_sym two (two_power state_p0)). *)
  Rewrite <- mult_assoc_l;Auto with arith.
  Rewrite  (invar_inv1 state_n0 state_p0).
  Rewrite e.
  Unfold shift .
  Auto with arith.
  Auto with arith.
...
(*  Rewrite (mult_sym two (two_power state_p0)). *)
  ...
  Case (lt_or_Zero m);Auto with arith.
  Intro.
  Absurd (lt one state_n0);Auto with arith.
  Rewrite e.
  Rewrite -> H1;Simpl;Auto with arith.
  Unfold stat_order ltof;Apply half_lt.
  Simpl.
  Apply lt_trans with one;Auto with arith.
  Auto with arith.
  Left.
  ...
  *)
  Program.
  Elim (invar_inv2 state_n0 state_p0).
  Program.
  Program.
  Program.

  Cut (lt (mult (two_power state_p0) state_n0) n0).
  Intro n0_left.

  Cut (lt n0 (mult (two_power state_p0) (S state_n0))).
  Intro n0_right.
 
  Cut (lt (mult (two_power (S state_p0)) m) n0).
  Intro n0_left'.

  Cut (lt n0 (mult (two_power (S state_p0)) (S m))).
  Intro n0_right'.
 ...
 (* Proof of n0_right':
  ...
  Simpl in b;Elim b;Intros eg.
  Apply lt_trans with (mult (two_power state_p0)
                            (S state_n0)) ;Auto with arith.
  Rewrite two_power_S.
  Rewrite <- mult_assoc_l;Auto with arith.
  Apply mult_lt_l.
  Auto with arith.
  Rewrite -> eg.
  Unfold shift ;Simpl;Auto with arith.
  Replace (mult (two_power (S state_p0)) (S m))  
   with   (mult (two_power state_p0) (S state_n0)).
  Elim (invar_inv2 state_n0 state_p0);Auto with arith.
  Elim H0;Auto with arith.
  Rewrite two_power_S.
  Rewrite <- mult_assoc_l.
 ...
 (* proof of n0_left':(lt (mult (two_power (S state_p0)) m) n0). *)
  ...
  Apply le_lt_trans with (mult (two_power state_p0) 
                                state_n0);Auto with arith.
  Simpl in b;Elim b;Intro eg.
  Rewrite eg.
  Rewrite  two_power_S.
  Rewrite <- mult_assoc_l;Auto with arith.
  ...
  (*   Proof of  n0_right:
          (lt n0 (mult (two_power state_p0) (S state_n0))) *)

   Elim (invar_inv2 state_n0 state_p0).
   Auto with arith.
   Elim H0;Auto with arith.
  
 (* Proof of n0_left:(lt (mult (two_power state_p0) state_n0) n0). *)
  ...
  Left.
  Rewrite (invar_inv1 ? ? H1).
  Rewrite (final_inv ? ? ? H0).
  Auto with arith.
  Auto with arith.
  ...
  Rewrite (final_inv ? ? ? H0).
  Intros.
  Rewrite <- (mult_n_1 (two_power state_p0));Auto with arith.
  (*  (lt n0 (two_power (S state_p0))) *)
  ...
  Simpl in H1;Rewrite  H1;Auto with arith.
  ...
  Simpl in H0.
  Simpl.
  Cut (ok (mkstate state_n0 state_p0 true)).
  Intro H'.
  Inversion_clear H'.
  Auto with arith.
  Elim H0;Auto with arith.
  Simpl in H0;Simpl.
  Cut (ok (mkstate state_n0 state_p0 false)).
  Intro H'.
  Inversion_clear H'.
  Auto with arith.
  Elim H0;Auto with arith.
